...
import { EventType } from "../event-type/shemas/event-type.schemas";
...
export class EventService {
  ...
  async create(createEventDto: CreateEventDto) {
    ...
    if (
      !isValidObjectId(eventTypeId) ||
      !isValidObjectId(venueId) ||
      !isValidObjectId(humanCategoryId) ||
      !isValidObjectId(langId)
    ) {
    ...
    const humanCategory =
      await this.humanCategorySchema.findById(humanCategoryId);
    if (!humanCategory) {
